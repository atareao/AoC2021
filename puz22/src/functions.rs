...
impl Cube{
    ...
    fn clone(&self) -> Cube{
        Cube::new(self.x, self.y, self.z)
    }

    fn add(&self, cube: Cube) -> Vec<Cube>{
        let mut resultado: Vec<Cube> = Vec::new();
        let mut x: (i32, i32) = (0, 0);
        let mut y: (i32, i32) = (0, 0);
        let mut z: (i32, i32) = (0, 0);
        if cube.x.1 < self.x.0 || cube.x.0 > self.x.1 ||
            cube.y.1 < self.y.0 || cube.y.0 > self.y.1 ||
            cube.z.1 < self.z.0 || cube.z.0 > self.z.1{
                resultado.push(self.clone());
                resultado.push(cube);
        }else if self.x.0 <= cube.x.0 && self.x.1 >= cube.x.1
            && self.y.0 <= cube.y.0 && self.y.1 >= cube.y.1
            && self.z.0 <= cube.z.0 && self.z.1 >= cube.z.1{
                resultado.push(self.clone());
        }else{
            x.0 = if cube.x.0 >= self.x.0 {cube.x.0} else {self.x.0};
            x.1 = if cube.x.1 <= self.x.1 {cube.x.1} else {self.x.1};
            y.0 = if cube.y.0 >= self.y.0 {cube.y.0} else {self.y.0};
            y.1 = if cube.y.1 <= self.y.1 {cube.y.1} else {self.y.1};
            z.0 = if cube.z.0 >= self.z.0 {cube.z.0} else {self.z.0};
            z.1 = if cube.z.1 <= self.z.1 {cube.z.1} else {self.z.1};
        }
        fn divide(&self, cube: Cube) -> Vec<Cube>{
            let mut resultado: Vec<Cube> = Vec::new();
            if cube.x
